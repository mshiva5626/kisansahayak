...
function renderBoldText(str) {
    ...
    if (str.includes('**')) {
        ...
        return parts.map((part, i) =>
            i % 2 === 1 ? (
                <strong key={i} className="font-bold text-slate-900 bg-emerald-100/70 px-1 py-0.5 rounded">
                    {part}
                </strong>
            ) : renderBoldText(part)
        );
    ...
    if (str.includes('*')) {
        ...
        return parts.map((part, i) =>
            i % 2 === 1 ? (
                <em key={i} className="italic text-emerald-800 font-medium">
                    {part}
                </em>
            ) : part
        );
...
/**
 * Parses markdown into sequential line blocks and structured table blocks
 */
function parseMarkdownBlocks(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Detect markdown table rows: starts with '|' and contains another '|' or has >= 3 pipes
        const isTableRow = (trimmed.startsWith('|') && trimmed.includes('|')) || 
                           (trimmed.endsWith('|') && trimmed.split('|').length >= 3);
        const isTableDivider = isTableRow && /^\|?[\s\-:|]+\|?$/.test(trimmed);
        ...
        if (isTableRow) {
            ...
            if (isTableDivider) {
                continue; // Skip divider row like |---|---|
            }
            const cleanTrimmed = trimmed.replace(/^\|/, '').replace(/\|$/, '');
            const cells = cleanTrimmed.split('|').map(c => c.trim());
            ...
            if (currentTable) {
                if (currentTable.headers.length > 0 && currentTable.rows.length > 0) {
                    ...
                    blocks.push({ type: 'line', content: '| ' + currentTable.headers.join(' | ') + ' |' });
                }
                currentTable = null;
            }
...
/**
 * Creative & Simple Agronomic Table Component
 * Renders data as sleek, modern mobile cards by default, with an instant toggle to classic table grid.
 */
const CreativeAgronomicTable = ({ headers = [], rows = [] }) => {
    const [viewMode, setViewMode] = useState('cards'); // 'cards' | 'grid'

    if (!headers || headers.length === 0 || !rows || rows.length === 0) return null;

    const primaryColHeader = headers[0] || 'Item';
    const detailHeaders = headers.slice(1);

    return (
        <div className="my-3.5 rounded-2xl bg-gradient-to-br from-slate-900/95 via-slate-900/90 to-emerald-950/40 border border-emerald-500/30 p-3.5 shadow-xl overflow-hidden">
            {/* Top Toolbar */}
            <div className="flex items-center justify-between gap-2 pb-2.5 mb-2.5 border-b border-white/10">
                <div className="flex items-center gap-2 min-w-0">
                    <span className="w-6 h-6 rounded-lg bg-emerald-500/20 text-emerald-400 flex items-center justify-center shrink-0 shadow-inner">
                        <span className="material-icons-round text-sm">view_agenda</span>
                    </span>
                    <span className="font-bold text-xs md:text-sm text-emerald-300 truncate">
                        {primaryColHeader} Overview
                    </span>
                    <span className="text-[10px] text-slate-300 bg-white/10 px-2 py-0.5 rounded-full border border-white/10 shrink-0 font-medium">
                ...
                <div className="flex items-center gap-1 bg-slate-100 p-0.5 rounded-xl border border-slate-200">
                    ...
                    >
                        <span className="material-icons-round text-xs">dashboard</span>
                        <span>Cards</span>
                    </button>
                    <button
                        type="button"
                        ...
                        title="Classic table grid"
                    >
                        <span className="material-icons-round text-xs">table_chart</span>
                        <span>Table</span>
                    </button>
                </div>
            </div>
            ...
            {/* Content View */}
            {viewMode === 'cards' ? (
                // 1. CREATIVE, CLEAN & SIMPLE CARDS
                <div className="space-y-2.5">
                    {rows.map((row, rIdx) => {
                        const title = row[0] || `Option ${rIdx + 1}`;
                        const metrics = [];
                        const notes = [];
                        ...
                        detailHeaders.forEach((h, hIdx) => {
                            ...
                            if (!val) return;
                            const isLong = val.length > 35 || 
                                /note|desc|guidance|advisory|detail|comment|remark/i.test(h);
                            if (isLong) {
                                notes.push({ label: h, text: val });
                            } else {
                                metrics.push({ label: h, text: val });
                            }
                        });
                        ...
                        return (
                            <div
                                key={rIdx}
                                className="rounded-xl bg-slate-50 hover:bg-slate-100/80 border border-slate-200 p-3.5 transition-all shadow-xs"
                            >
                                {/* Card Title Header */}
                                <div className="flex items-center gap-2">
                                    <span className="w-2 h-2 rounded-full bg-emerald-600 shrink-0 shadow-xs"></span>
                                    <h5 className="font-bold text-sm text-emerald-900">
                                        {renderBoldText(title)}
                                    </h5>
                                </div>

                                {/* Key Metrics / Quantitative Values (Chips Grid) */}
                                {metrics.length > 0 && (
                                    <div className="flex flex-wrap gap-2 mt-2.5">
                                        {metrics.map((m, mIdx) => (
                                            <div
                                                key={mIdx}
                                                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-white border border-slate-200 text-xs shadow-xs"
                                            >
                                ...
                                {/* Long Notes / Guidance Section */}
                                {notes.length > 0 && (
                                    <div className="mt-2.5 pt-2 border-t border-slate-200 space-y-1.5">
                                        {notes.map((n, nIdx) => (
                                            <div key={nIdx} className="text-xs text-slate-700 leading-relaxed flex items-start gap-2 bg-white p-2 rounded-lg border border-slate-200">
                                                <span className="material-icons-round text-emerald-600 text-sm shrink-0 mt-0.5">
                                                    lightbulb
                                                </span>
                                                <div className="flex-1">
                                                    ...
                                                    <span>{renderBoldText(n.text)}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-xs">
                    <table className="w-full text-left border-collapse text-xs">
                        <thead>
                            <tr className="bg-emerald-50 border-b border-slate-200 text-emerald-900 font-bold uppercase tracking-wider text-[10px]">
                                {headers.map((h, hIdx) => (
                                        ...
                                        return (
                                            <td
                                                ...
                                                className={`px-3 py-2.5 text-slate-700 ${
                                                    cIdx === 0
                                                        ? 'font-bold text-emerald-800 whitespace-nowrap'
                                                        : 'leading-relaxed'
                                                }`}
                                            >
                                                {renderBoldText(cell)}
...
function renderAgronomicMarkdown(text, isAI) {
    ...
    return blocks.map((block, idx) => {
        ...
        if (trimmed.startsWith('###') || (trimmed.startsWith('**') && trimmed.endsWith('**') && trimmed.length < 60) || /^\d+\.\s+\*\*.*?\*\*/.test(trimmed)) {
            ...
            return (
                <div key={idx} className="mt-3 mb-1.5 flex items-center gap-2 border-b border-emerald-100 pb-1">
                    <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
                    <h4 className="font-bold text-sm md:text-base text-emerald-900 tracking-tight">{headerText}</h4>
                </div>
            );
        }
        ...
        if (trimmed.toLowerCase().includes('caution') || trimmed.toLowerCase().includes('safety') || trimmed.toLowerCase().includes('warning') || trimmed.toLowerCase().includes('phi:')) {
            return (
                <div key={idx} className="my-2 p-2.5 rounded-2xl bg-amber-50 border border-amber-200 flex items-start gap-2 text-xs text-amber-900 shadow-xs">
                    <span className="material-icons-round text-amber-600 text-sm shrink-0 mt-0.5">gpp_maybe</span>
        ...
        if (trimmed.toLowerCase().includes('cost per acre') || trimmed.toLowerCase().includes('estimated cost') || trimmed.toLowerCase().includes('₹/acre')) {
            return (
                <div key={idx} className="my-2 p-2 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center gap-2 text-xs text-emerald-800 font-bold">
                    <span className="material-icons-round text-emerald-600 text-sm">currency_rupee</span>
        ...
        if (/^\d+\./.test(trimmed)) {
            ...
            return (
                <div key={idx} className="flex items-start gap-2.5 my-1.5 ml-0.5 text-sm">
                    <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-800 font-black text-[11px] flex items-center justify-center shrink-0 mt-0.5 shadow-xs">
                        {stepNum}
                    </span>
        ...
        if (trimmed.startsWith('- ') || trimmed.startsWith('* ') || trimmed.startsWith('• ')) {
            ...
            return (
                <div key={idx} className="flex items-start gap-2 my-1 ml-1.5 text-sm text-slate-700">
                    <span className="text-emerald-600 font-bold text-sm shrink-0 mt-0.5">•</span>
        ...
        return (
            <p key={idx} className="text-sm leading-relaxed text-slate-700 my-1">
                {renderBoldText(line)}
            </p>
        );
    });
...
const ChatMessageItem = React.memo(({
    ...
    return (
        <div className={`flex gap-3 items-end ${msg.isAI ? '' : 'flex-row-reverse'}`}>
            {msg.isAI && (
                <div className="w-8 h-8 rounded-2xl bg-emerald-100 border border-emerald-200 flex items-center justify-center self-end mb-1 text-emerald-700 shrink-0">
            ...
            <div className={`flex flex-col gap-1 max-w-[88%] md:max-w-[78%] ${msg.isAI ? '' : 'items-end'}`}>
                <div className={`${
                    msg.isAI 
                        ? 'bg-white border border-slate-200/90 text-slate-800 rounded-3xl rounded-bl-sm shadow-xs' 
                        : 'bg-emerald-600 text-white rounded-3xl rounded-br-sm shadow-xs font-normal'
                } p-4 md:p-5 relative transition-all`}>
                    
                    {msg.attachment && (
                        <div className="mb-3 p-2 rounded-2xl bg-slate-50 border border-slate-200 flex items-center gap-3">
                            ...
                            <div className="overflow-hidden text-xs">
                                <p className="font-bold truncate text-slate-900">{msg.attachment.name}</p>
                                <p className="text-[10px] text-slate-500">{msg.attachment.sizeKb} KB • Uploaded for Diagnosis</p>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="flex items-center gap-1.5">
                                {msg.sources && msg.sources.length > 0 && (
                                    <button
                                        ...
                                        className="px-2.5 py-1 rounded-xl bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-800 text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer"
                                        title="View multi-source verified intelligence"
                                    >
                                        <span className="material-icons-round text-xs text-emerald-600">verified</span>
                                ...
                                <button
                                    onClick={() => onCopy(msg)}
                                    className="p-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 transition-colors cursor-pointer flex items-center gap-1 text-[11px]"
                                    title="Copy text"
                                >
                                    <span className="material-icons-round text-sm">
                                        {isCopied ? 'check' : 'content_copy'}
                                    </span>
                                    {isCopied && <span className="text-emerald-700 font-bold">Copied!</span>}
                                </button>
                                ...
                                <button
                                    ...
                                    className={`p-1.5 rounded-xl transition-all cursor-pointer text-[11px] ${
                                        isPlaying
                                            ? 'bg-emerald-100 text-emerald-800 font-bold border border-emerald-200 animate-pulse'
...
const AIAdvisoryChatbot = ({ onBack, selectedFarmId, userProfile, chatContext, clearContext }) => {
    ...
    return (
        <div className="relative w-full h-screen bg-[#f8fafc] text-slate-800 font-display antialiased flex flex-col overflow-hidden selection:bg-emerald-100 selection:text-emerald-900">
            ...
            {isDrawerOpen && (
                <div className="fixed inset-0 z-50 flex">
                    ...
                    <div className="relative w-80 max-w-[85vw] h-full bg-white/95 border-r border-slate-200/90 p-5 flex flex-col shadow-2xl z-10 backdrop-blur-xl animate-slide-in-left text-slate-800">
                        <div className="flex items-center justify-between pb-4 border-b border-slate-200">
                            <div className="flex items-center gap-2.5">
                                ...
                                <h3 className="font-bold text-sm text-slate-900">Chat Memory</h3>
                            </div>
                            <button
                                onClick={() => setIsDrawerOpen(false)}
                        ...
                        <button
                            ...
                            className="mt-4 w-full py-3 px-4 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
                        >
                        ...
                        {farmData && (
                            <div className="mt-4 p-3 rounded-2xl bg-emerald-50/80 border border-emerald-200/70 flex items-center gap-3">
                                <div className="w-9 h-9 rounded-xl bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0">
                                    <span className="material-icons-round text-lg">agriculture</span>
                                </div>
                                <div className="overflow-hidden">
                                    <p className="text-[11px] font-extrabold text-emerald-800 uppercase tracking-wider">Active Farm Context</p>
                                    <p className="text-xs font-bold text-slate-900 truncate">{farmData.farm_name || 'My Farm'} • {farmData.crop_type || 'Crop'}</p>
                                    <p className="text-[10px] text-slate-500 truncate">{farmData.district || farmData.state || 'India'}</p>
                                </div>
                            </div>
                        )}
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-all cursor-pointer"
            ...
            {activeSourcesModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-fade-in">
                    <div className="bg-white border border-slate-200 rounded-3xl p-6 w-full max-w-lg shadow-2xl relative max-h-[88vh] flex flex-col text-slate-800">
                        <div className="flex items-center justify-between pb-3 border-b border-slate-200">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <h3 className="font-bold text-base text-slate-900">Multi-Source Intelligence</h3>
                                    <p className="text-[11px] text-emerald-700 font-semibold">6 Authoritative Agricultural Pillars Synthesized</p>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 my-4 pr-1">
                            {activeSourcesModal.map((src, i) => (
                                <div key={i} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 hover:border-emerald-300 transition-all space-y-1.5">
                                    <div className="flex items-center justify-between">
                                        <h5 className="text-xs font-bold text-emerald-850 flex items-center gap-1.5">
                                    ...
                                    <p className="text-[11px] font-bold text-slate-700">{src.org}</p>
                                    <p className="text-[11px] text-slate-600 leading-relaxed bg-white border border-slate-200/60 p-2.5 rounded-xl">{src.detail}</p>
                                </div>
                            ))}
                        </div>
                        ...
                        <button
                            onClick={() => setActiveSourcesModal(null)}
                            className="w-full py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-all cursor-pointer shadow-md shadow-emerald-600/20"
                        >
            ...
            {/* Reaction Toast */}
            {reactionToast && (
                <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-4 py-2 rounded-2xl shadow-xl text-xs font-bold animate-bounce-short backdrop-blur-lg">
                    {reactionToast}
                </div>
            )}

            {/* 4. Top Navigation Bar */}
            <header className="sticky top-0 z-20 px-4 pt-12 pb-3.5 bg-white/95 border-b border-slate-200/90 backdrop-blur-xl shadow-xs">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => setIsDrawerOpen(true)}
                            className="p-2 -ml-2 rounded-2xl hover:bg-slate-100 text-slate-700 transition-colors cursor-pointer relative"
                        ...
                        <div className="flex items-center gap-2.5">
                            <div className="relative">
                                <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-emerald-100 to-teal-100 border border-emerald-200 flex items-center justify-center text-emerald-700 shadow-sm">
                                    <span className="material-icons-round text-xl">smart_toy</span>
                                </div>
                                <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-500 border-2 border-white rounded-full"></div>
                            </div>
                            <div>
                                <h1 className="font-bold text-sm md:text-base text-slate-900 leading-tight flex items-center gap-1.5">
                                    <span>Kisan Copilot</span>
                                    <span className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-800 border border-emerald-200">Ultra-Fast Pro</span>
                                </h1>
                    ...
                    <button
                        onClick={handleNewChat}
                        className="p-2 px-3 rounded-2xl bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 hover:text-slate-900 transition-colors cursor-pointer text-xs font-bold flex items-center gap-1"
                        title="New Chat"
                    >
                        <span className="material-icons-round text-base text-emerald-600">add</span>
            ...
            <main ref={scrollRef} className="flex-1 overflow-y-auto p-4 pb-44 space-y-5 scroll-smooth no-scrollbar relative z-10">
                ...
                {isTyping && (
                    <div className="flex gap-3 items-center ml-11 animate-fade-in">
                        <div className="flex items-center gap-2.5 bg-white border border-emerald-200/90 py-2.5 px-4 rounded-2xl shadow-sm backdrop-blur-md">
                            ...
                            <span className="text-xs font-bold text-emerald-800 ml-1">Cross-referencing ICAR, CIBRC, IMD & Agmarknet...</span>
                        </div>
                    </div>
                )}
            </main>
            ...
            <div className="fixed bottom-24 left-0 w-full px-4 overflow-x-auto no-scrollbar flex space-x-2 z-20 pointer-events-auto">
                {[
                    ...
                    <button
                        ...
                        className="px-3.5 py-2 rounded-2xl bg-white/95 border border-slate-200/90 hover:border-emerald-400 hover:bg-emerald-50/70 text-xs font-semibold text-slate-700 hover:text-emerald-800 whitespace-nowrap shadow-sm backdrop-blur-md active:scale-95 transition-all cursor-pointer"
                    >
                        {q}
                    </button>
                ))}
            ...
            {/* 7. Bottom Input Bar */}
            <div className="fixed bottom-0 left-0 w-full bg-white/95 border-t border-slate-200/90 pb-6 pt-2.5 z-30 shadow-[0_-8px_24px_rgba(0,0,0,0.06)] backdrop-blur-2xl">
                {attachment && (
                    <div className="max-w-md mx-auto px-4 mb-2">
                        <div className="p-2.5 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center justify-between gap-3 shadow-xs animate-fade-in">
                            <div className="flex items-center gap-2.5 overflow-hidden">
                                ...
                                <div className="overflow-hidden">
                                    ...
                                    <p className="text-[10px] text-emerald-700 font-semibold">{attachment.sizeKb} KB • Multi-Source Lab & Image Analysis Ready</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setAttachment(null)}
                                className="p-1.5 rounded-full hover:bg-emerald-100 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer"
                                title="Remove file"
                            >
                ...
                <div className="max-w-md mx-auto px-4 flex items-end gap-2">
                    ...
                    <button
                        ...
                        className="flex-shrink-0 w-11 h-11 rounded-2xl bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-600 hover:text-emerald-700 transition-all flex items-center justify-center cursor-pointer active:scale-95 disabled:opacity-50"
                    ...
                    <div className={`flex-1 rounded-2xl px-3.5 py-2.5 flex items-center gap-2 border transition-all duration-200 ${
                        isListening 
                            ...
                            : 'bg-slate-100/90 border-slate-200 focus-within:border-emerald-500 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-500/10'
                    }`}>
                        <textarea
                            className="w-full bg-transparent border-none p-0 text-sm text-slate-900 placeholder-slate-400 focus:outline-none resize-none max-h-24 font-normal"
                            placeholder={
                                isListening 
                                    ? 'Listening to speech...' 
                                    : attachment 
                        ...
                        <button
                            ...
                            className={`flex-shrink-0 p-1.5 rounded-full transition-colors cursor-pointer ${
                                isListening ? 'text-red-500 bg-red-100 animate-pulse' : 'text-slate-400 hover:text-slate-700'
                            }`}
                    ...
                    <button
                        ...
                        className="flex-shrink-0 w-11 h-11 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white shadow-md shadow-emerald-600/20 flex items-center justify-center hover:scale-105 active:scale-95 transition-all disabled:opacity-40 disabled:scale-100 cursor-pointer"
                        title="Send Message"
                    >
